...
// TestDestinations_MultiTable_Append_SchemaEvolution validates that multi-table append
// applies schema evolution when source tables gain new columns.
func TestDestinations_MultiTable_Append_SchemaEvolution(t *testing.T) {
	if testing.Short() {
		t.Skip("Skipping integration test in short mode")
	}
	...
	usersFile1 := createMultiTableTestFile(t, "users_append_evolve_initial", []map[string]interface{}{
		{"id": 1, "name": "alice", "active": true},
		{"id": 2, "name": "bob", "active": false},
	})
	defer func() { _ = os.Remove(usersFile1) }()

	ordersFile1 := createMultiTableTestFile(t, "orders_append_evolve_initial", []map[string]interface{}{
		{"id": 1, "user_id": 1, "amount": 100.50},
	})
	defer func() { _ = os.Remove(ordersFile1) }()

	usersFile2 := createMultiTableTestFile(t, "users_append_evolve_updated", []map[string]interface{}{
		{"id": 3, "name": "charlie", "active": true, "email": "charlie@example.com"},
		{"id": 4, "name": "david", "active": true, "email": "david@example.com"},
	})
	defer func() { _ = os.Remove(usersFile2) }()

	ordersFile2 := createMultiTableTestFile(t, "orders_append_evolve_updated", []map[string]interface{}{
		{"id": 2, "user_id": 3, "amount": 50.00},
	})
	defer func() { _ = os.Remove(ordersFile2) }()
	...
	for _, tc := range multiTableDestinationCases() {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			p1 := pipeline.New(cfg)
			require.NoError(t, p1.Run(ctx), "Initial multi-table replace should succeed")

			cfg.SourceURI = sourceURI2
			cfg.IncrementalStrategy = config.StrategyAppend

			p2 := pipeline.New(cfg)
			require.NoError(t, p2.Run(ctx), "Multi-table append with new column should succeed")
			...
			var ordersCount int
			require.NoError(t, db.QueryRow(tc.sqlBackend.countQuery(ordersTable)).Scan(&ordersCount))
			assert.Equal(t, 2, ordersCount, "orders table should have 2 rows after append")
		})
	}
}
