...
- **Monetary values are loaded as strings.** Flexi's `numeric` type carries money, and the exact decimal text is preserved rather than being converted to a float. Cast explicitly downstream to the scale you want.
