...
ZIP archives use `!` to separate the S3 object pattern from a glob that selects archive members:

```sh
ingestr ingest \
    --source-uri 's3://?access_key_id=AKC3YOW7E&secret_access_key=XCtkpL5B' \
    --source-table 'my_bucket/releases/*.zip!**/*.csv' \
    --dest-uri duckdb:///compressed_data.duckdb \
    --dest-table 'release.events'
```

CSV, JSONL/NDJSON, and parquet members are detected by their names. Append a format hint when selected members do not have useful extensions, for example `my_bucket/release.zip!data/*#jsonl`. If `!<member-glob>` is omitted, all files in the ZIP are selected and unsupported formats are skipped.

Every ZIP row includes the outer object path and member metadata:

- `_ingestr_source_file_path`
- `_ingestr_source_archive_member_path`
- `_ingestr_source_archive_member_crc32`
- `_ingestr_source_archive_member_compressed_size`
- `_ingestr_source_archive_member_uncompressed_size`

ZIP objects are spooled to a bounded temporary file so the archive is not loaded into memory, then selected members are processed one at a time. The following source URI parameters configure safety limits: `archive_max_members` (default `10000`), `archive_max_bytes` (default 10 GiB), `archive_max_uncompressed_bytes` (default 100 GiB), and `archive_max_expansion_ratio` (default `1000`). Unsafe member paths are rejected.
