...
ZIP archives are read like other compressed files. Match the ZIP objects directly:

```sh
ingestr ingest \
    --source-uri 's3://?access_key_id=AKC3YOW7E&secret_access_key=XCtkpL5B' \
    --source-table 'my_bucket/releases/*.zip' \
    --dest-uri duckdb:///compressed_data.duckdb \
    --dest-table 'release.events'
```

CSV, JSONL/NDJSON, and parquet files inside each ZIP are detected by their names. Append a format hint when files do not have useful extensions, for example `my_bucket/release.zip#jsonl`. Unsupported files are skipped.

ZIP objects are spooled to a temporary file so the archive is not loaded into memory, then supported files are processed one at a time.
