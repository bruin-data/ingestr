...
```plaintext
adls://?account_name=<storage-account>&account_key=<account-key>
```
...
If `account_key` and `sas_token` are omitted, `ingestr` uses Azure's default credential chain. This supports environment variables, managed identity, Azure CLI login, and other credentials supported by `DefaultAzureCredential`.
...
```sh
ingestr ingest \
    --source-uri 'adls://?account_name=myaccount&account_key=<account-key>' \
...
ingestr ingest \
    ...
    --dest-uri 'adls://?account_name=myaccount&account_key=<account-key>' \
...
ingestr ingest \
    --source-uri 'duckdb:///records.db' \
    --source-table 'public.users' \
    --dest-uri 'adls://?account_name=myaccount&account_key=<account-key>&layout={table_name}/{load_id}.{file_id}.{ext}' \
    --dest-table 'lakehouse/records'
```
