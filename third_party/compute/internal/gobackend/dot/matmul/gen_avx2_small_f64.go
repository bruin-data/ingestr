...
func avx2SmallFloat64Parallel( //alt:f64
	...
	if maxWorkers <= 1 || batchSize <= 1 {
		if layout == dot.LayoutNonTransposed {
			...
			avx2SmallFloat64(lhs, rhs, 0, batchSize, lhsCrossSize, rhsCrossSize, contractingSize, output) //alt:f64
		} else {
			...
			avx2SmallFloat64Transposed(lhs, rhs, 0, batchSize, lhsCrossSize, rhsCrossSize, contractingSize, output) //alt:f64
		}
		return
	}
	...
	backend.Workers.Saturate(func() {
		for chunk := range work {
			if layout == dot.LayoutNonTransposed {
				...
				avx2SmallFloat64(lhs, rhs, chunk.batchIdx, chunk.batchCount, lhsCrossSize, rhsCrossSize, contractingSize, output) //alt:f64
			} else {
				...
				avx2SmallFloat64Transposed(lhs, rhs, chunk.batchIdx, chunk.batchCount, lhsCrossSize, rhsCrossSize, contractingSize, output) //alt:f64
			}
		}
	})
}
...
func avx2SmallFloat64( //alt:f64
	...
	//alt:f32|bf16|f16 var oZero float32
	...
	for range batchCount {
		...
		for ; row+3 < lhsCrossSize; row += 4 {
			for k := 0; k < contractingSize; k++ {
				...
				l0 := archsimd.BroadcastFloat64x4(*(*float64)(unsafe.Pointer(l0_ptr))) //alt:f64
				...
				l1 := archsimd.BroadcastFloat64x4(*(*float64)(unsafe.Pointer(l1_ptr))) //alt:f64
				...
				l2 := archsimd.BroadcastFloat64x4(*(*float64)(unsafe.Pointer(l2_ptr))) //alt:f64
				...
				l3 := archsimd.BroadcastFloat64x4(*(*float64)(unsafe.Pointer(l3_ptr))) //alt:f64

				rColBase := rhsBase + uintptr(k*rhsCrossSize)*iSize
				//alt:f32|bf16|f16 const outputVecWidth = 8
				const outputVecWidth = 4 //alt:f64
				...
				for ; col+2*outputVecWidth <= rhsCrossSize; col += 2 * outputVecWidth {
					...
					//alt:f32|bf16|f16 var o0_0, o0_1, o1_0, o1_1, o2_0, o2_1, o3_0, o3_1 archsimd.Float32x8
				...
				for ; col+outputVecWidth <= rhsCrossSize; col += outputVecWidth {
					...
					//alt:f32|bf16|f16 var o0_0, o1_0, o2_0, o3_0 archsimd.Float32x8
				...
				for ; col < rhsCrossSize; col++ {
					...
					if k == 0 {
						//alt:f32|bf16|f16 *(*float32)(unsafe.Pointer(outBase0)) = l0_scalar * r0
						//alt:f32|bf16|f16 *(*float32)(unsafe.Pointer(outBase1)) = l1_scalar * r0
						//alt:f32|bf16|f16 *(*float32)(unsafe.Pointer(outBase2)) = l2_scalar * r0
						//alt:f32|bf16|f16 *(*float32)(unsafe.Pointer(outBase3)) = l3_scalar * r0
		...
		for ; row < lhsCrossSize; row++ {
			for k := 0; k < contractingSize; k++ {
				...
				l0 := archsimd.BroadcastFloat64x4(*(*float64)(unsafe.Pointer(l_ptr))) //alt:f64
				...
				for ; col < rhsCrossSize; col++ {
					...
					outBase := outputBase + uintptr(row*rhsCrossSize+col)*oSize
					if k == 0 {
						//alt:f32|bf16|f16 *(*float32)(unsafe.Pointer(outBase)) = l0_scalar * r0
...
func avx2SmallFloat64Transposed( //alt:f64
	//alt:f32 lhs, rhs []float32,
	//alt:bf16  lhs, rhs []bfloat16.BFloat16,
	//alt:f16  lhs, rhs []float16.Float16,
	lhs, rhs []float64, //alt:f64
	...
	for range batchCount {
		for row := 0; row < lhsCrossSize; row++ {
			for col := 0; col < rhsCrossSize; col++ {
				...
				//alt:f32|bf16|f16 acc := archsimd.BroadcastFloat32x8(0.0)
				acc := archsimd.BroadcastFloat64x4(0.0) //alt:f64
				k := 0
				...
				//alt:f32|bf16|f16 res := avx2ReduceSumFloat32x8(acc)
				res := avx2ReduceSumFloat64x4(acc) //alt:f64
				...
				//alt:f32|bf16|f16 *(*float32)(unsafe.Pointer(outputIdx)) = res
