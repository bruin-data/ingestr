...
// - Base source file (edit this one): avx2_large.go
...
func avx2LargeFloat64( //alt:f64
	...
	params := AVX2ParamsFloat64 //alt:f64
	...
	if maxWorkers <= 1 {
		...
		for range batchSize {
			...
			avx2LargeMatrixSliceFloat64( //alt:f64
				layout,
...
func avx2LargeMatrixSliceFloat64( //alt:f64
	layout dot.Layout,
	...
	if params.LHSL1KernelRows != 4 || params.RHSL1KernelCols != 8 { //alt:f64
		...
		panic(errors.Errorf("unsupported kernel L1 block sizes for avx2 kernel: lhsL1BlockRows=%d, rhsL1BlockCols=%d, wanted 4 and 8 respectively (params=%+v)", //alt:f64
	...
	for rhsPanelColIdx := colStart; rhsPanelColIdx < colEnd; rhsPanelColIdx += params.RHSPanelCrossSize {
		...
		for contractingPanelIdx := 0; contractingPanelIdx < contractingSize; contractingPanelIdx += params.PanelContractingSize {
			...
			for lhsPanelRowIdx := rowStart; lhsPanelRowIdx < rowEnd; lhsPanelRowIdx += params.LHSPanelCrossSize {
				...
				avx2LargeKernelFloat64( //alt:f64
					packedLHS, packedRHS, packedOutput,
				...
				avx2ApplyPackedOutputFloat64( //alt:f64
...
func avx2LargeKernelFloat64( //alt:f64
	...
	const (
		...
		//alt:f32|bf16|f16 kernelCols = 16
		kernelCols = 8 //alt:f64
		//alt:f32|bf16|f16 outputNumLanes = 8
		outputNumLanes = 4 //alt:f64
	...
	for lhsRowIdx := 0; lhsRowIdx < lhsActiveRows; lhsRowIdx += kernelRows {
		...
		for rhsColIdx := 0; rhsColIdx < rhsActiveCols; rhsColIdx += kernelCols {
			...
			//alt:f32|bf16|f16 accum_lhs0_rhs0 := archsimd.BroadcastFloat32x8(0.0)
			accum_lhs0_rhs0 := archsimd.BroadcastFloat64x4(0.0) //alt:f64
			//alt:f32|bf16|f16 accum_lhs0_rhs1 := archsimd.BroadcastFloat32x8(0.0)
			accum_lhs0_rhs1 := archsimd.BroadcastFloat64x4(0.0) //alt:f64
			//alt:f32|bf16|f16 accum_lhs1_rhs0 := archsimd.BroadcastFloat32x8(0.0)
			accum_lhs1_rhs0 := archsimd.BroadcastFloat64x4(0.0) //alt:f64
			//alt:f32|bf16|f16 accum_lhs1_rhs1 := archsimd.BroadcastFloat32x8(0.0)
			accum_lhs1_rhs1 := archsimd.BroadcastFloat64x4(0.0) //alt:f64
			//alt:f32|bf16|f16 accum_lhs2_rhs0 := archsimd.BroadcastFloat32x8(0.0)
			accum_lhs2_rhs0 := archsimd.BroadcastFloat64x4(0.0) //alt:f64
			//alt:f32|bf16|f16 accum_lhs2_rhs1 := archsimd.BroadcastFloat32x8(0.0)
			accum_lhs2_rhs1 := archsimd.BroadcastFloat64x4(0.0) //alt:f64
			//alt:f32|bf16|f16 accum_lhs3_rhs0 := archsimd.BroadcastFloat32x8(0.0)
			accum_lhs3_rhs0 := archsimd.BroadcastFloat64x4(0.0) //alt:f64
			//alt:f32|bf16|f16 accum_lhs3_rhs1 := archsimd.BroadcastFloat32x8(0.0)
			accum_lhs3_rhs1 := archsimd.BroadcastFloat64x4(0.0) //alt:f64
			...
			for range contractingLen {
				...
				//alt:f32|bf16|f16 lhsVec0 := archsimd.BroadcastFloat32x8(lhsVal0)
				lhsVec0 := archsimd.BroadcastFloat64x4(lhsVal0)            //alt:f64
				...
				//alt:f32|bf16|f16 lhsVec1 := archsimd.BroadcastFloat32x8(lhsVal1)
				lhsVec1 := archsimd.BroadcastFloat64x4(lhsVal1)            //alt:f64
				...
				//alt:f32|bf16|f16 lhsVec2 := archsimd.BroadcastFloat32x8(lhsVal2)
				lhsVec2 := archsimd.BroadcastFloat64x4(lhsVal2)            //alt:f64
				...
				//alt:f32|bf16|f16 lhsVec3 := archsimd.BroadcastFloat32x8(lhsVal3)
				lhsVec3 := archsimd.BroadcastFloat64x4(lhsVal3)            //alt:f64
