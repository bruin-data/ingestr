...
def db_to_db_merge_with_primary_key(
    ...
    if "clickhouse" in dest_connection_url:
        count_by_run_id = dest_engine.execute(
            f'select _dlt_load_id, count(*) from "{database_name}"."{schema_rand_prefix}.output".output group by 1 order by 2 desc'
        ).fetchall()
    else:
        count_by_run_id = dest_engine.execute(
    ...
    if "clickhouse" in dest_connection_url:
        count_by_run_id = dest_engine.execute(
            f'select _dlt_load_id, count(*) from "{database_name}"."{schema_rand_prefix}.output".output group by 1'
        ).fetchall()
    else:
        count_by_run_id = dest_engine.execute(
    ...
    if "clickhouse" in dest_connection_url:
        count_by_run_id = dest_engine.execute(
            f'select _dlt_load_id, count(*) from "{database_name}"."{schema_rand_prefix}.output".output group by 1 order by 2 desc, 1 asc'
        ).fetchall()
    else:
        count_by_run_id = dest_engine.execute(
