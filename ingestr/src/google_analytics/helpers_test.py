...
def test_convert_minutes_ranges_to_int_list():
    user_input = "1-2,5-6"
    expected_result = [MinuteRange(name="1-2 minutes ago", start_minutes_ago=2), MinuteRange(name="5-6 minutes ago", start_minutes_ago=6)]
    ...
    with pytest.raises(ValueError, match=expected_result_2):convert_minutes_ranges_to_int_list(user_input_2)

    user_input_3 = "12,56"
    expected_result_3 = "Invalid input. Minutes range should be startminute-endminute format. For example: 1-2,5-6"
    with pytest.raises(ValueError, match=expected_result_3):convert_minutes_ranges_to_int_list(user_input_3)

    user_input_4 = "1-2,5-"
    expected_result_4 = "Invalid input. Minutes range should be startminute-endminute format. For example: 1-2,5-6"
    with pytest.raises(ValueError, match=expected_result_4):convert_minutes_ranges_to_int_list(user_input_4)
