...
class SqlSource:
    ...
    def dlt_source(self, uri: str, table: str, **kwargs):
        ...
        if uri.startswith("db2://"):
            ...
            if not username:
                raise ValueError("Username is required to connect to the DB2 database.")
            if not password:
                raise ValueError("Password is required to authenticate with the DB2 database.")
            if not host:
                raise ValueError("The hostname or IP address of the DB2 server is required to establish a connection.")
            if not port:
                raise ValueError("The TCP port of the DB2 server is required to establish a connection.")
            if not database:
                raise ValueError("The database name is required to connect to the DB2 database.")
