    ...
    def dlt_source(self, uri: str, table: str, **kwargs):
        ...
        if ":" in table:
            ...
            if not isinstance(query, list):
                raise ValueError("Query must be a JSON array representing a MongoDB aggregation pipeline")
            ...
            if kwargs.get("incremental_key"):
                ...
                self._validate_incremental_query(query, incremental_key)
        ...
        for stage in query:
            if "$project" in stage:
                ...
                if isinstance(project_stage, dict):
                    ...
                    elif all(v in [0, False] for v in project_stage.values() if v in [0, False, 1, True]):
                        # This is an exclusion projection, incremental_key is included by default
                        if incremental_key not in project_stage:
                            incremental_key_projected = True
                        else:
                            incremental_key_projected = project_stage[incremental_key] not in [0, False]
                    else:
    ...
    def _substitute_interval_params(self, query: list, kwargs: dict):
        ...
        if interval_start is not None:
            if isinstance(interval_start, str):
                ...
                interval_start = pendulum_dt.to_datetime() if hasattr(pendulum_dt, 'to_datetime') else pendulum_dt
        ...
        if interval_end is not None:
            if isinstance(interval_end, str):
                ...
                interval_end = pendulum_dt.to_datetime() if hasattr(pendulum_dt, 'to_datetime') else pendulum_dt
