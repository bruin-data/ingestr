...
DEFAULT_METRICS = ["Displays", "Clicks", "AdvertiserCost", "Ctr", "Cpc", "Cpm"]
...
class CriteoAPI:
    def __init__(
        self, client_id: str, client_secret: str, access_token: Optional[str] = None
    ):
        """
        ...
        self.request_client = create_client()
    ...
    def fetch_campaign_statistics(
        ...
        if currency not in SUPPORTED_CURRENCIES:
            raise ValueError(
                f"Unsupported currency: {currency}. Supported currencies: {SUPPORTED_CURRENCIES}"
            )
    ...
    def validate_dimensions_and_metrics(
        self, dimensions: list[str], metrics: list[str]
    ) -> bool:
        """
        ...
        if invalid_dimensions:
            raise ValueError(
                f"Invalid dimensions: {invalid_dimensions}. Available dimensions: {AVAILABLE_DIMENSIONS}"
            )

        invalid_metrics = [m for m in metrics if m not in AVAILABLE_METRICS]
        if invalid_metrics:
            raise ValueError(
                f"Invalid metrics: {invalid_metrics}. Available metrics: {AVAILABLE_METRICS}"
            )
