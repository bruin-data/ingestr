...
def app_store(
    ...
    return [
        ...
        app_downloads_standard(client, app_ids)
    ]

def get_analytics_report(client: AppStoreConnectClient, app_id: str, report_name: str) -> Iterable[TDataItem]:
...
@dlt.resource(name="app-downloads-detailed", columns={"counts": {"data_type": "bigint"}}, primary_key=["date", "app_id"])
def app_downloads_detailed(client: AppStoreConnectClient, app_ids: List[str]) -> Iterable[TDataItem]:
    for app_id in app_ids:
        yield from get_analytics_report(client, app_id, "App Downloads Detailed")

@dlt.resource(name="app-downloads-standard")
def app_downloads_standard(client: AppStoreConnectClient, app_ids: List[str]) -> Iterable[TDataItem]:
    for app_id in app_ids:
        yield from get_analytics_report(client, app_id, "App Downloads Standard")