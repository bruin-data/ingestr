...
func (d *DuckDBDestination) SwapTable(ctx context.Context, opts destination.SwapOptions) error {
	...
	if stagingSchema == targetSchema && len(opts.PrimaryKeys) == 0 {
		// Same schema and no dedup needed: cheap rename swap. When primary keys
		// are set we fall through to the recreate+copy path below, which is the
		// only place we can deduplicate (a rename can't) and re-apply the PK.
