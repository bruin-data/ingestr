...
func (d *SnowflakeDestination) LoadCDCState(ctx context.Context, table, connectorID string) ([]destination.CDCStateEntry, error) {
	...
	query := fmt.Sprintf("SELECT %s, %s, %s, %s, %s, %s, %s FROM %s WHERE %s = ?",
		quoteIdentifier("event_id"), quoteIdentifier("source_table"), quoteIdentifier("destination_table"), quoteIdentifier("state_kind"), quoteIdentifier("state_generation"),
...
func (d *SnowflakeDestination) LoadCDCStateFence(ctx context.Context, table, connectorID string) (destination.CDCStateFence, error) {
	ctx = d.annotate(ctx, annotation.StepCDCResume)
	quotedTable := quoteFQN(sfTable(table))
	query := fmt.Sprintf("SELECT DISTINCT %s, %s FROM %s WHERE %s = ? AND %s = 'run' AND %s = (SELECT MAX(%s) FROM %s WHERE %s = ? AND %s = 'run') ORDER BY %s",
		quoteIdentifier("event_id"), quoteIdentifier("state_generation"), quotedTable,
		quoteIdentifier("connector_id"), quoteIdentifier("state_kind"), quoteIdentifier("state_generation"),
		quoteIdentifier("state_generation"), quotedTable, quoteIdentifier("connector_id"), quoteIdentifier("state_kind"), quoteIdentifier("event_id"))
...
func (d *SnowflakeDestination) DeleteCDCStateEvents(ctx context.Context, table, connectorID string, eventIDs []string) error {
	...
	query := fmt.Sprintf("DELETE FROM %s WHERE %s = ? AND %s IN (%s)", quoteFQN(sfTable(table)), quoteIdentifier("connector_id"), quoteIdentifier("event_id"), strings.Join(placeholders, ", "))
