...
func TestAdaptyByteCap(t *testing.T) {
	wide := strings.Repeat("x", 2048)
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		rows := []map[string]interface{}{}
		for i := 0; i < 50; i++ {
			rows = append(rows, map[string]interface{}{"placement_id": i, "name": wide})
		}
		w.Header().Set("Content-Type", "application/json")
	...
	run := func(max int64) (int64, int64) {
		s := &AdaptySource{analyticsClient: testAdaptyClient(srv.URL)}
		results, err := s.readPlacements(context.Background(), tableParams{}, source.ReadOptions{MaxBatchBytes: max})
		if err != nil {
			t.Fatal(err)
		}
		var b, rw int64
