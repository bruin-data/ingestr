...
func TestClickUpByteCap(t *testing.T) {
	...
	run := func(max int64) (int64, int64) {
		s := &ClickUpSource{client: ingestrhttp.New(ingestrhttp.WithBaseURL(srv.URL))}
		results, err := s.read(context.Background(), "teams", source.ReadOptions{MaxBatchBytes: max})
		if err != nil {
			t.Fatal(err)
		}
		var b, rw int64
