...
func TestGranolaByteCap(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if calls == 1 {
			for i := 0; i < 50; i++ {
				rows = append(rows, map[string]interface{}{"id": fmt.Sprintf("folder-%d", i), "name": wide})
			}
		}
		w.Header().Set("Content-Type", "application/json")
		_ = json.NewEncoder(w).Encode(map[string]interface{}{
	...
	run := func(max int64) (int64, int64) {
		calls = 0
		s := &GranolaSource{client: ingestrhttp.New(ingestrhttp.WithBaseURL(srv.URL))}
		results, err := s.read(context.Background(), "folders", source.ReadOptions{MaxBatchBytes: max})
		if err != nil {
			t.Fatal(err)
		}
		var b, rw int64
