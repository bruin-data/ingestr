...
func TestTrelloByteCap(t *testing.T) {
	wide := strings.Repeat("x", 2048)
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		rows := make([]map[string]interface{}, 0, 50)
		for i := 0; i < 50; i++ {
			rows = append(rows, map[string]interface{}{"id": fmt.Sprintf("%d", i), "name": wide})
		}
	...
	run := func(max int64) (int64, int64) {
		s := &TrelloSource{client: httpclient.New(httpclient.WithBaseURL(srv.URL))}
		results, err := s.read(context.Background(), "boards", source.ReadOptions{MaxBatchBytes: max})
		if err != nil {
			t.Fatal(err)
		}
		var b, rw int64
