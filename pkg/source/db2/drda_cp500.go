...
var cp500Encode = func() map[rune]byte {
	encoded := make(map[rune]byte, len(cp500Decode))
	for i, r := range cp500Decode {
		if _, exists := encoded[r]; !exists {
			encoded[r] = byte(i)
		}
	}
	return encoded
...
func encodeCP500(value string) ([]byte, error) {
	out := make([]byte, 0, len(value))
	for _, r := range value {
		b, ok := cp500Encode[r]
		if !ok {
			return nil, fmt.Errorf("cp500 cannot encode %q", r)
		}
		out = append(out, b)
	}
	return out, nil
...
func decodeCP500(value []byte) string {
	var b strings.Builder
	b.Grow(len(value))
	for _, v := range value {
		b.WriteRune(cp500Decode[v])
	}
	return b.String()
}
