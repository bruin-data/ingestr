...
func TestWistiaByteCap(t *testing.T) {
	wide := strings.Repeat("x", 2048)
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		items := []map[string]interface{}{}
		if r.URL.Query().Get("page") == "1" {
			for i := 0; i < 50; i++ {
				items = append(items, map[string]interface{}{"visitor_key": i, "name": wide})
			}
		}
		w.Header().Set("Content-Type", "application/json")
	...
	run := func(max int64) (int64, int64) {
		...
		results, err := s.read(context.Background(), "stats_visitors", source.ReadOptions{MaxBatchBytes: max})
		if err != nil {
			t.Fatal(err)
		}
		var b, rw int64
