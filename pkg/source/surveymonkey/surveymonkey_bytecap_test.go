...
func TestSurveyMonkeyByteCap(t *testing.T) {
	...
	run := func(max int64) (int64, int64) {
		s := &SurveyMonkeySource{client: httpclient.New(httpclient.WithBaseURL(srv.URL))}
		results, err := s.read(context.Background(), "contact_lists", source.ReadOptions{MaxBatchBytes: max})
		if err != nil {
			t.Fatal(err)
		}
		var b, rw int64
