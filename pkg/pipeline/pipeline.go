...
func (p *Pipeline) Run(ctx context.Context) (retErr error) {
	...
	if p.config.Stream {
		...
		if !ok || !ss.SupportsStreaming() {
			return fmt.Errorf("--stream is not supported by this source; streaming requires a CDC source (postgres+cdc, mysql+cdc/mariadb+cdc, vitess+cdc, mssql+cdc, mongodb+cdc) or a message broker source")
		}
