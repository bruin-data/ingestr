...
- **Sized string columns**: a `STRING(n)`/`BYTES(n)` bound (from a source `varchar(n)` or a `--columns` override) rides on `schema.Column.MaxLength`. `BuildBigQuerySchema` writes it to `FieldSchema.MaxLength` on create, and **`GetTableSchema` reads it back** — the read-back must preserve it, otherwise an unchanged sized column looks unbounded to schema evolution and a subsequent incremental run derives an unbounded prepared schema that no longer matches the bounded table. When the target already exists, `validateBigQuerySchemaCompatibility` fail-fasts on an incompatible existing column (a bounded column asked to become unbounded, a bound narrower than required, or insufficient NUMERIC precision/scale) rather than silently truncating. Widening a bounded string in place is *not* auto-applied — a wider requirement is rejected, not `ALTER`ed (the one exception is the CDC `_cdc_lsn` state column, which is explicitly widened to unbounded). The compat check does **not** compare column type *kinds* at all — a differing type (e.g. existing `INTEGER`, desired `FLOAT`) is left to schema evolution. This is deliberate: `PrepareTable` (and its check) runs with the *final* schema but **before** the strategy calls `job.ApplyEvolution`, so the physical column is still the old type when the check runs; if the check rejected the difference, the widening `ALTER` that resolves it would never run. Evolution reconciles the type via `ALTER … SET DATA TYPE` using the same `CanWiden`/`GetWidenedType` contract (or fails loudly if BigQuery can't perform the coercion) — re-deriving that here would be redundant. The reported case (table created `INTEGER`, this run infers `FLOAT`) is verified live end-to-end: the column is widened `INTEGER→FLOAT` and `1.5` is preserved, not truncated. What the check *does* still reject: a `Repeated` (scalar↔array) mismatch, which is not a widening evolution can perform. The parameterized bound check (STRING/BYTES max length, NUMERIC precision/scale) runs only when the types match — it is the fail-fast for a *bounded* column that would need to become unbounded/wider, which evolution does **not** auto-apply (skill note above) and which the write would otherwise only surface non-deterministically when an over-long value happens to arrive.
