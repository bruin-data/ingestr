...
Replace flows through a staging table:
...
- **Sized string columns**: a `STRING(n)`/`BYTES(n)` bound (from a source `varchar(n)` or a `--columns` override) rides on `schema.Column.MaxLength`. `BuildBigQuerySchema` writes it to `FieldSchema.MaxLength` on create, and **`GetTableSchema` reads it back** — the read-back must preserve it, otherwise an unchanged sized column looks unbounded to schema evolution and a subsequent incremental run derives an unbounded prepared schema that no longer matches the bounded table. When the target already exists, `validateBigQuerySchemaCompatibility` fail-fasts on an incompatible existing column (a bounded column asked to become unbounded, a bound narrower than required, or insufficient NUMERIC precision/scale) rather than silently truncating. Widening a bounded string in place is *not* auto-applied — a wider requirement is rejected, not `ALTER`ed (the one exception is the CDC `_cdc_lsn` state column, which is explicitly widened to unbounded). On a differing column *type kind* (e.g. existing `INTEGER`, desired `FLOAT`) the check's behavior depends on whether schema evolution has a plan to reconcile it, via the `deferTypeChanges` flag:

- **Table existed at evolution-planning time** (`reconcileExistingTable(..., true)`): the difference is left to schema evolution, which widens the column via `ALTER … SET DATA TYPE` using the same `CanWiden`/`GetWidenedType` contract (or fails loudly if BigQuery can't coerce). Re-deriving that here would be redundant — and rejecting would be wrong, because `PrepareTable` runs with the *final* schema but **before** the strategy calls `job.ApplyEvolution`, so the physical column is still the old type when the check runs; rejecting would kill the very `ALTER` that resolves it. Verified live: existing `INTEGER` + inferred `FLOAT` widens to `FLOAT` with `1.5` preserved.
- **Concurrent-create-winner path** (`reconcileExistingTable(..., false)`): a peer created the table *after* evolution planning saw it absent, so **no plan exists** to alter a mismatching column. Here a type-kind difference is **rejected** (the run fails and a retry re-plans against the now-existing table) rather than deferred to a nonexistent `ALTER`, which would otherwise let the write hit an incompatible physical column (silent truncation via the merge cast).

Independent of that flag, the check always rejects a `Repeated` (scalar↔array) mismatch (not a widening evolution can perform), and always runs the parameterized bound check when the types match: a *bounded* column that would need to become unbounded/wider is still a hard fail-fast (STRING/BYTES max length, NUMERIC precision/scale), because widening a bounded string in place is *not* auto-applied (the one exception is the CDC `_cdc_lsn` state column, explicitly widened to unbounded) and the write would otherwise only surface it non-deterministically when an over-long value arrives.
